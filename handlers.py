...
async def send_welcome(message: types.Message):
    ...
    key = message.text.split(" ")[1] if len(message.text.split(" ")) > 1 else ""

    logger.info(f"User {username} (ID: {user_id}) started the bot with key: {key}")
    ...
    if "error" in response_data and "error_msg" in response_data:
        ...
        logger.info(f"User {username} (ID: {user_id}) authorized successfully.")


@router_main.message(F.text == "Список рейсов на сегодня")
async def handle_button1(message: types.Message, bot: Bot):
    from keyboard import run_chats, send_routes

    user_id = message.chat.id
    if str(user_id) not in run_chats:
        logger.warning(f"Not authorized user trying to use keyboard: {user_id}")
